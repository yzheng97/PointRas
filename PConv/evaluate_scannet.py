...
parser.add_argument('--ply_path', default='/opt/data2/scannet/scans', help='ply path from original Scannet')
...
def evaluate(num_votes):
    with tf.device('/gpu:'+str(GPU_INDEX)):
        ...
        pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, NUM_CLASSES, BANDWIDTH, mode="inference") # rend
        #MODEL.get_loss(pred, labels_pl, smpws_pl, mode="inference")
...
def eval_one_epoch(sess, ops, num_votes=1, topk=1):
    ...
    for batch_idx in tqdm(range(num_batches)):
        ...
        whole_scene_points = TEST_DATASET_WHOLE_SCENE.scene_points_list[batch_idx]
        ...
        _, uvlabel, _ = pc_util.point_cloud_label_to_surface_voxel_label_fast(whole_scene_points, np.concatenate((np.expand_dims(whole_scene_label,1),np.expand_dims(pred_label,1)),axis=1), res=0.02)
    ...
    OA = np.sum(np.array(total_correct_class[1:])) / np.sum(np.array(total_seen_class[1:],dtype=np.float))
    VOA = np.sum(total_correct_class_vox[1:])/(np.sum(total_seen_class_vox[1:])*1.0)
    log_string('Overall acc: %f' % (OA))
    log_string('Overall voxel acc: %f' % (VOA))
    print("Done!")
