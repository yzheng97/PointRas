...
__global__ void gatherpointKernel(int b,int c,int n,int m,const float * __restrict__ inp,const int * __restrict__ idx,float * __restrict__ out){
  for (int i=blockIdx.x;i<b;i+=gridDim.x){
    for (int j=blockIdx.y*blockDim.x+threadIdx.x;j<m;j+=blockDim.x*gridDim.y){
      int a=idx[i*m+j];
      for (int k=0;k<c;k+=1){
        out[(i*m+j)*c+k]=inp[(i*n+a)*c+k];
      }
    }
  }
}

__global__ void scatteraddpointKernel(int b,int c,int n,int m,const float * __restrict__ out_g,const int * __restrict__ idx,float * __restrict__ inp_g){
  for (int i=blockIdx.x;i<b;i+=gridDim.x){
    for (int j=blockIdx.y*blockDim.x+threadIdx.x;j<m;j+=blockDim.x*gridDim.y){
      int a=idx[i*m+j];
      for (int k=0;k<c;k+=1){
        atomicAdd(&inp_g[(i*n+a)*c+k],out_g[(i*m+j)*c+k]);
      }
    }
  }
}
...
void gatherpointLauncher(int b,int c,int n,int m,const float * inp,const int * idx,float * out){
  gatherpointKernel<<<dim3(2,8,1),512>>>(b,c,n,m,inp,idx,out);
}
void scatteraddpointLauncher(int b,int c,int n,int m,const float * out_g,const int * idx,float * inp_g){
  scatteraddpointKernel<<<dim3(2,8,1),512>>>(b,c,n,m,out_g,idx,inp_g);
}
