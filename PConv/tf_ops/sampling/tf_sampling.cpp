...
class GatherPointGpuOp: public OpKernel{
  public:
    ...
    void Compute(OpKernelContext * context)override{
      ...
      gatherpointLauncher(b,c,n,m,inp,idx,out);
    }
};
...
class GatherPointGradGpuOp: public OpKernel{
  public:
    ...
    void Compute(OpKernelContext * context)override{
      ...
      OP_REQUIRES(context,out_g_tensor.dims()==3 && out_g_tensor.shape().dim_size(0)==b && out_g_tensor.shape().dim_size(1)==m && out_g_tensor.shape().dim_size(2)==c,errors::InvalidArgument("GatherPointGradGpuOp expects (batch_size,num_result,3) out_g shape"));
      ...
      scatteraddpointLauncher(b,c,n,m,out_g,idx,inp_g);
    }
};
