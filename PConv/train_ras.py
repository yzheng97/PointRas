...
parser.add_argument('--model', default='pointconv_rend', help='Model name [default: model]')
parser.add_argument('--log_dir', default='pointconv_scannet_rend_', help='Log dir [default: log]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            # Get model and loss 
            pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, NUM_CLASSES, BANDWIDTH, bn_decay=bn_decay, mode="train")
            loss = MODEL.get_loss(pred, labels_pl, smpws_pl, end_points, mode="train")
            pred_eval, _ = MODEL.get_model(pointclouds_pl, is_training_pl, NUM_CLASSES, BANDWIDTH, bn_decay=bn_decay, mode="inference")
            loss_eval = MODEL.get_loss(pred, labels_pl, smpws_pl, end_points, mode="inference")
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if epoch % 10 == 0:
                save_path = saver.save(sess, os.path.join(LOG_DIR, "model_%03d.ckpt"%(epoch)))
                log_string("Model saved in file: %s" % save_path)
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        summary, step, loss_val, pred_val = sess.run([ops['merged'], ops['step'],
            ops['loss_eval'], ops['pred_eval']], feed_dict=feed_dict)
...
def eval_whole_scene_one_epoch(sess, ops, test_writer):
    ...
    log_string('eval whole point avg class IoU: %f' % (mIoU))
