...
from PointConv import feature_encoding_layer_rend, feature_decoding_layer, interpolate_module, interpolate_module_nn
...
def get_model(point_cloud, is_training, num_class, sigma, bn_decay=None, weight_decay=None, mode="train"):
    ...
    if mode == "train":
        net = tf_util.conv1d(l3_points, 64, 1, padding='VALID', bn=True, is_training=is_training, scope='fc3', bn_decay=bn_decay)
        ...
        l_xyz = [l0_xyz, l1_xyz, l2_xyz, l3_xyz]
        #l_points = [l0_points, l1_points, l2_points, l3_points]
        l3_feats = l3_points
        l2_feats = interpolate_module(l2_xyz, l3_xyz, l3_feats)
        l1_feats = interpolate_module(l1_xyz, l2_xyz, l2_feats)
        l0_feats = interpolate_module(l0_xyz, l1_xyz, l1_feats)
        l_points = [l0_feats, l1_feats, l2_feats, l3_feats]
        l_idx = [l0_idx, l1_idx, l2_idx, l3_idx]
        net = tf_util.conv1d(l3_points, 64, 1, padding='VALID', bn=True, is_training=is_training, scope='fc3', bn_decay=bn_decay)
        net = tf_util.dropout(net, keep_prob=0.5, is_training=is_training, scope='dp2')
        l3_mask = tf_util.conv1d(net, num_class, 1, padding='VALID', activation_fn=None, scope='fc4') # l3_mask

        l2_coarse_mask = interpolate_module(l_xyz[2], l_xyz[3], l3_mask)
        l2_uncertain_idx = get_uncertainty_idx(l2_coarse_mask, l2_coarse_mask.shape[1]//2)
        l2_uncertain_idx_in_all = get_idx_in_all(l_idx[2], l2_uncertain_idx)
        l2_uncertain_mask = gather_point(l2_coarse_mask, l2_uncertain_idx)
        l2_final_uncertain_mask = gather_point(final_mask, l2_uncertain_idx_in_all)
        l2_uncertain_feats = gather_point(l_points[2], l2_uncertain_idx)
        l2_combined_feats = tf.concat(axis=2, values=[l2_uncertain_feats, l2_uncertain_mask, l2_final_uncertain_mask])
        l2_rend = tf_util.conv1d(l2_combined_feats, num_class, 1, padding='VALID', activation_fn=None, scope='rend_mlp')
        l2_mask = batch_scatter_add(l2_coarse_mask, l2_uncertain_idx, l2_rend-l2_uncertain_mask)

        l1_coarse_mask = interpolate_module(l_xyz[1], l_xyz[2], l2_mask)
        l1_uncertain_idx = get_uncertainty_idx(l1_coarse_mask, l1_coarse_mask.shape[1]//2)
        l1_uncertain_idx_in_all = get_idx_in_all(l_idx[1], l1_uncertain_idx)
        l1_uncertain_mask = gather_point(l1_coarse_mask, l1_uncertain_idx)
        l1_final_uncertain_mask = gather_point(final_mask, l1_uncertain_idx_in_all)
        l1_uncertain_feats = gather_point(l_points[1], l1_uncertain_idx)
        l1_combined_feats = tf.concat(axis=2, values=[l1_uncertain_feats, l1_uncertain_mask, l1_final_uncertain_mask])
        l1_rend = tf_util.conv1d(l1_combined_feats, num_class, 1, padding='VALID', activation_fn=None, scope='rend_mlp')
        l1_mask = batch_scatter_add(l1_coarse_mask, l1_uncertain_idx, l1_rend-l1_uncertain_mask)

        l0_coarse_mask = interpolate_module(l_xyz[0], l_xyz[1], l1_mask)
        l0_uncertain_idx = get_uncertainty_idx(l0_coarse_mask, l0_coarse_mask.shape[1]//2)
        l0_uncertain_idx_in_all = l0_uncertain_idx
        l0_uncertain_mask = gather_point(l0_coarse_mask, l0_uncertain_idx)
        l0_final_uncertain_mask = gather_point(final_mask, l0_uncertain_idx_in_all)
        l0_uncertain_feats = gather_point(l_points[0], l0_uncertain_idx)
        l0_combined_feats = tf.concat(axis=2, values=[l0_uncertain_feats, l0_uncertain_mask, l0_final_uncertain_mask])
        l0_rend = tf_util.conv1d(l0_combined_feats, num_class, 1, padding='VALID', activation_fn=None, scope='rend_mlp')
        # both l0_coarse_mask and final_mask can be ori in batch_scatter
        l0_mask = batch_scatter_add(l0_coarse_mask, l0_uncertain_idx, l0_rend-l0_uncertain_mask)

        final_uncertain_idx = get_uncertainty_idx(final_mask, final_mask.shape[1]//6)
        delta = gather_point(l0_mask, final_uncertain_idx)-gather_point(final_mask, final_uncertain_idx)
        final_mask = batch_scatter_add(final_mask, final_uncertain_idx, delta)
...
def get_uncertainty_idx_v2(mask, final_mask, k):
    B = mask.shape[0]
    uncertainty_map = tf.norm(mask - final_mask, axis=2)
    _, idx = tf.nn.top_k(tf.reshape(uncertainty_map, (B,-1)), k)
    return idx
